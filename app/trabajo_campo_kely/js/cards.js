...
function organizarActividadesPorTipo(actividades) {
    ...
    Object.keys(resultado).forEach(tipo => {
        resultado[tipo].sort((a, b) => {
            const fechaB = new Date(`${b.fecha_actividad}T00:00:00`);
            const fechaA = new Date(`${a.fecha_actividad}T00:00:00`);
...
function crearCard(ac, color) {
    ...
    const [year, month, day] = ac.fecha_actividad.split("-");
...
document.addEventListener("click", async function (e) {
    if (e.target.closest(".btn-anular")) {
        ...
        if (resultado.isConfirmed) {
            ...
            try {
                const response = await fetch("../controller/actividadescontrol.php?accion=anular", {
                    method: 'POST',
                    headers: {
