...
class EncuestaSatisfaccion {
    constructor() {
        this.apiUrl = '../controller/EncuestaController.php';
        this.votos = {};
        this.preguntas = [];
    ...
    async cargarEncuesta() {
        try {
            const response = await fetch(`${this.apiUrl}?action=obtener&cod_tipo_form=3`);
    ...
    obtenerColorClass(enunciado) {
        ...
        if (enunciado.includes('Muy Insatisfecho')) return 'muy-insatisfecho';
        return 'neutral';
    }
    ...
    async enviarEncuesta() {
        ...
        try {
            const payload = {
                cod_territorio: this.codTerritorio,
